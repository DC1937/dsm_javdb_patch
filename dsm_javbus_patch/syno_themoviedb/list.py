...
host="https://www.dmmsee.zone/"
def javlist(title):
    # 获取有码信息
    url = host+ "search/"+title+"&type=&parent=ce"
    r = requests.get(url)
    ...
    for i in range(len(list)):
        ...
        vmsg['summary'] = title
        title = id + " "+ title
        vmsg['url'] = list[i].select(".movie-box")[0].get("href").replace(host,"")
        ...
        data.append(vmsg)
    #获取无码信息
    url = host + "uncensored/search/"+title+"&type=&parent=ce"
    r = requests.get(url)
    r = r.content.decode("utf-8")
    soup = BeautifulSoup(r,"lxml")
    list = soup.select("#waterfall .item")
